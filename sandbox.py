...
# fill onto w/ individuals
def translate_article_to_ontology(article):
    
    onto = get_ontology(ontology_local_link).load()
    # onto = get_ontology("file:///owlready_onto_i.owl").load()

    authors = article['authors']
    
    article_title = article['title'].replace(' ', '_')
    # article_i = onto.Articles(article_title)
    article_i = onto.Articles('Science_2')

    journal_title = article['journal_ref'].replace(' ', '_')
    journal_i = onto.Journals(journal_title)
    # onto.save()
    onto.save(file = ontology_local_link)
    """
    for author in authors:
        '''
        author_i = onto.Authors(author['name'].replace(' ', '_'))
        lab_i = onto.Authors(author['lab'].replace(' ', '_'))
        print(lab_i)
        author_i.wrote_article.append(article_i)
        author_i.works_in.append(lab_i)
        ...
        author_text = 'Mister_X'
        author_i = onto.Authors(author_text)
        '''
        
        author_j = onto.Authors('mister_x')
        author_k = onto.Authors('mister_y')
        author_j.works_in.append(author_k)
        
        # author_j = 'Zelle, J. M.'
        # author_j.is_quoted_by.append(someone_2)
    """
    # onto.save(file = ontology_local_link)

    # onto.save(file = ontology_local_link_output)
